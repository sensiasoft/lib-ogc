...
public class PolygonTypeBinding extends AbstractComplexBinding {
    ...
    public Object parse(ElementInstance instance, Node node, Object value)
        ...
        Polygon poly = gFactory.createPolygon(exterior, interior);
        
        if (node.hasAttribute("srsName")) {
            String srs = ((URI)node.getAttributeValue("srsName")).toString();
            int strPos = srs.lastIndexOf('/'); // case of URL
            if (strPos < 0)
                strPos = srs.lastIndexOf(':'); // case of URN
            poly.setSRID(Integer.parseInt(srs.substring(strPos+1)));
        }
    ...
    public Object getProperty(Object object, QName name)
        ...
        if ("srsName".equals(name.getLocalPart())) {
            return "http://www.opengis.net/def/crs/EPSG/0/" + polygon.getSRID();
        }
        
        if ("id".equals(name.getLocalPart()))
            return "P" + idFormatter.format(currentId++);
